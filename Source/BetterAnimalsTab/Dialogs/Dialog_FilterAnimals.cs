...
*/
		...
		public Dialog_FilterAnimals()
		{
			...
			{
				_pawnKinds = Find.VisibleMap.mapPawns.AllPawnsSpawned
								 .Where(x => x.RaceProps.Animal && x.Faction == null)
								 .Select(x => x.kindDef)
								 .Distinct()
								 .OrderBy(x => x.LabelCap)
								 .ToList();
		...
		private void DrawFilterRow( Filter filter )
		{
			...
			if ( filter != null )
			{
				label.Append( ( filter.Author + "." + filter.Label + "Label" ).Translate() ).Append( " " );
				var rect = new Rect( _x, _y, ColWidth, _rowHeight );
				...
				switch ( filter.State )
				{
					case FilterType.True:
						...
						label.Append( "(" ).Append( ( filter.Author + "." + filter.Label + "Yes" ).Translate() ).Append( ")" );
						break;

					case FilterType.False:
						GUI.DrawTexture( rectIcon, filter.Textures[1] );
						label.Append( "(" ).Append( (filter.Author + "." + filter.Label + "No" ).Translate() ).Append( ")" );
						break;
